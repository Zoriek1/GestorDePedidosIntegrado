...
class AuditLog(db.Model):
    ...
    actor = db.Column(
        db.String(100),
        nullable=True,
        comment="Usuário/cliente/terminal que executou a ação",
    )
    action = db.Column(
        db.String(50),
        nullable=False,
        index=True,
        comment="CREATE/UPDATE/DELETE/RESTORE/OVERRIDE_DELETE",
    )
    entity_type = db.Column(
        db.String(50),
        nullable=False,
        index=True,
        comment="Tipo de entidade: pedido, cliente, etc",
    )
    entity_id = db.Column(db.Integer, nullable=True, index=True, comment="ID da entidade afetada")
    metadata_json = db.Column(
        db.Text,
        nullable=True,
        comment="JSON ou texto com informações adicionais (diffs, resumo)",
    )
