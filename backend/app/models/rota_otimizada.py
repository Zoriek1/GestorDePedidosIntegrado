...
class RotaOtimizada(db.Model):
    ...
    sequencia_pedidos = db.Column(db.Text, nullable=False, comment='JSON array com IDs dos pedidos na ordem otimizada')
    ...
    waypoints_coords = db.Column(db.Text, nullable=True, comment='JSON array com coordenadas dos waypoints na ordem otimizada')
    
    # Metadados
    num_pedidos = db.Column(db.Integer, nullable=False, comment='Número de pedidos na rota')
    metodo_otimizacao = db.Column(db.String(50), nullable=True, comment='Método usado (exata, nearest_neighbor, etc)')
    ...
    def __repr__(self):
        return f'<RotaOtimizada #{self.id} - {self.num_pedidos} pedidos, {self.distancia_total_km} km>'
    ...
    def get_rotas_ativas():
        ...
        return RotaOtimizada.query.filter_by(ativa=True).order_by(RotaOtimizada.created_at.desc()).all()
