...
def capig_autoconfig():
    ...
    try:
        ...
        if token:
            ...
            print(f"[META_GATEWAY] Token na query string detectado - retornando JSON")
...
<body>
    ...
    <pre id="result"></pre>
        ...
        # Log para debug
        print(f"[META_GATEWAY] Autoconfig chamado")
        ...
        traceback.print_exc()
        response = jsonify({
            "error": "Erro ao processar autoconfiguração",
            "message": str(e)
        })
        response.headers["Content-Type"] = "application/json; charset=utf-8"
        return _add_cors_headers(response), 500
...
def meta_gateway_events(pixel_id):
    ...
    try:
        ...
        if not payload:
            response = jsonify({
                "error": "Payload vazio",
                "message": "É necessário enviar um payload JSON"
            })
            response.headers["Content-Type"] = "application/json; charset=utf-8"
            return _add_cors_headers(response), 400
