    ...
    def _calcular_por_faixas(self, distancia_km: float, config: Dict) -> float:
        ...
        if usa_faixas_intervalo:
            ...
            for i, faixa in enumerate(faixas):
                ...
                if ate_km is None:
                    ...
                    # Se a distância está acima desta faixa, verificar próxima faixa
                        ...
                        if i + 1 < len(faixas):
                            ...
                            if distancia_km < de_prox_km:
                                ...
                                if self.DEBUG:
                                    print(
                                        f"[DEBUG] Distância {distancia_km} km entre faixas, usando próxima: {de_prox_km}-{ate_prox_km} km -> R$ {taxa_prox:.2f}"
                                    )
                                return taxa_prox
                    ...
                    # Se a distância está antes da primeira faixa, usar a primeira faixa
                        ...
                        if self.DEBUG:
                            print(
                                f"[DEBUG] Distância {distancia_km} km antes da primeira faixa, usando primeira: {de_km}-{ate_km} km -> R$ {taxa:.2f}"
                            )
                        return taxa
            ...
            if faixas:
                ...
                if self.DEBUG:
                    print(
                        f"[DEBUG] Distância {distancia_km} km acima de todas as faixas, usando última: R$ {taxa_fallback:.2f}"
                    )
            ...
            if faixas_ordenadas:
                ...
                if self.DEBUG:
                    print(
                        f"[DEBUG] Nenhuma faixa encontrada, usando última: R$ {taxa_fallback:.2f}"
                    )
                return taxa_fallback
        ...
        if usa_faixas_intervalo:
            ...
            for faixa in sorted(
                ...
                if ate_km is None:
                    return (
                        descricao
                        or f"Acima de {faixas[-2].get('ate_km') if len(faixas) > 1 else 0} km"
                    )
                elif distancia_km <= ate_km:
