                    ...
                    if not validation_result.success:
                        ...
                        if preventive_backup and preventive_backup.exists():
                            ...
                            try:
                                ...
                                print(
                                    f"[ERRO] Falha ao restaurar backup preventivo: {rollback_error}"
                                )
...
def main():
    ...
    print()
    sucesso = restore_mgr.restore_backup(backup_to_restore, create_backup_first=not args.no_backup)
    ...
    if sucesso:
        ...
        try:
            if AUDIT_LOGGER_AVAILABLE:
                ...
                basic_logger.warning(f"RESTORE SUCESSO | Arquivo: {backup_to_restore.name}")
        except Exception:
            pass
        ...
        try:
            if AUDIT_LOGGER_AVAILABLE:
                ...
                basic_logger.error(f"RESTORE FALHA | Arquivo: {backup_to_restore.name}")
        except Exception:
            pass
