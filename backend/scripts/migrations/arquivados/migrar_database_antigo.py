...
try:
    ...
    from app.models import Pedido, Cliente, FontePedido, RotaOtimizada, EnderecoCliente
...
class DatabaseMigrator:
    ...
    def copy_table_data(self, old_conn: sqlite3.Connection, new_conn: sqlite3.Connection, 
        ...
        if existing_count > 0:
            ...
            if not self.overwrite:
                ...
                try:
                    response = input(f"  Deseja sobrescrever? (s/n): ").lower()
                    if response != 's':
    ...
    def migrate(self, tables_to_migrate: List[str] = None) -> Dict[str, int]:
        ...
        if not self.new_db_path.exists():
            ...
            print(f"[INFO] Criando banco novo...")
