...
def normalize_valores_pedidos(auto_confirm=False):
    ...
    backup_path = (
        db_path.parent
        / f"database_backup_before_valor_migration_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    )
    ...
    try:
        ...
        if not auto_confirm:
            try:
                ...
                print(
                    "[ERRO] Nao foi possivel obter confirmacao. Use --auto para execucao automatica."
                )
                return False
    ...
    try:
        ...
        for pedido_id, valor_original in precisam_conversao:
            try:
                ...
                if valor_float is None:
                    print(
                        f"[AVISO] Pedido #{pedido_id}: Nao foi possivel converter '{valor_original}'"
                    )
                    falhas += 1
                    continue

                valor_novo = format_valor_to_string(valor_float)
                cursor.execute("UPDATE pedidos SET valor = ? WHERE id = ?", (valor_novo, pedido_id))
                convertidos += 1
