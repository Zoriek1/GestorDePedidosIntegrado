...
from scripts.backup.drive_utils import (  # noqa: E402
...
class TestDriveUtils(unittest.TestCase):
    ...
    def test_check_drive_separation_same_drive_warning(self, mock_get_drive):
        ...
        self.assertTrue(
            any("mesmo drive" in w.lower() or "same drive" in w.lower() for w in warnings)
        )
    ...
    def test_check_drive_separation_secondary_same_as_db_warning(self, mock_get_drive):
        ...
        self.assertGreater(
            len(warnings), 0, f"Esperado pelo menos 1 warning, mas obteve: {warnings}"
        )
        # Verificar que há warning sobre secondary (pode ser "secundário" ou "secondary")
        has_secondary_warning = any(
            "secondary" in w.lower() or "secundário" in w.lower() for w in warnings
        )
        self.assertTrue(
            has_secondary_warning,
            f"Esperado warning sobre secondary/secundário, mas obteve: {warnings}",
        )
